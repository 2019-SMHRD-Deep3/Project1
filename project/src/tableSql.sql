drop table HYGIENE CASCADE CONSTRAINTS ;
drop table RIDER  CASCADE CONSTRAINTS;
drop table SHOP  CASCADE CONSTRAINTS;
drop table USER2  CASCADE CONSTRAINTS;
drop table ORDER2 CASCADE CONSTRAINTS;
DROP TABLE MENU CASCADE CONSTRAINTS;
DROP TABLE DELIVERY CASCADE CONSTRAINTS;
DROP TABLE REVIEW CASCADE CONSTRAINTS;
DROP TABLE REPLY CASCADE CONSTRAINTS;
DROP TABLE FOODCATEGORY CASCADE CONSTRAINTS;
DROP TABLE MENUDETAILS CASCADE CONSTRAINTS;

 CREATE TABLE USER2 
 (USER_ID VARCHAR2 (10) PRIMARY KEY,
 PW VARCHAR2 (15),
 USERNAME VARCHAR2 (5),
 USER_ADDRESS VARCHAR2(20),
 USER_PHONE NUMBER (11),
 CONSTRAINT USER2_PHONE_UK UNIQUE (USER_PHONE)
 );

CREATE TABLE SHOP 
(SHOP_ID VARCHAR2 (10) PRIMARY KEY,
PW VARCHAR2 (15),
SHOPNAME VARCHAR2 (10),
SHOP_ADDRESS VARCHAR2(20),
BUSINESS_LICENSE VARCHAR2 (10) ,
SHOP_TEL NUMBER(11),
SECTOR CHAR(2),
CONSTRAINT SHOP_BUSINESS_LICENSE_UK UNIQUE(BUSINESS_LICENSE),
CONSTRAINT SHOP_SHOP_TEL_UK UNIQUE(SHOP_TEL),
CONSTRAINT SHOP_SECTOR_FK FOREIGN KEY (SECTOR)
REFERENCES FOODCATEGORY(UNQ_NUMBER)
 );
 
 CREATE TABLE RIDER 
 (RIDER_ID VARCHAR2 (10) PRIMARY KEY,
 PW VARCHAR2(15),
 RIDER_NAME VARCHAR2(5),
 TRANSPORTATION VARCHAR2(20),
 LOC VARCHAR2(5),
 COMPANY VARCHAR2 (10),
 RIDER_PHONE NUMBER (11),
 CONSTRAINT RIDER_RIDER_PHONE_UK UNIQUE(RIDER_PHONE)
);

CREATE TABLE REVIEW 
(REVIEW_SEQ NUMBER(5) PRIMARY KEY,
USER_ID VARCHAR2 (10),
SHOP_ID VARCHAR2 (10),
RIDEER_ID VARCHAR2 (10),
SHOP_RATING  number (1),
RIDER_RATING number (1),
REVIEW varchar2(30),
CONSTRAINT REVIEW_USER_ID_FK FOREIGN KEY(USER_ID)
REFERENCES USER2(USER_ID),
CONSTRAINT REVIEW_RIDEER_ID_FK FOREIGN KEY (RIDEER_ID)
REFERENCES RIDER(RIDEER_ID)
);
 
 
CREATE TABLE ORDER2
(ORDER_NUMBER NUMBER (7) PRIMARY KEY,
USER_ID  VARCHAR2 (10) ,
SHOP_ID VARCHAR2 (10),
ORDERED_SEQ NUMBER (7),
CONSTRAINT ORDER2_USER_ID_FK FOREIGN KEY (USER_ID)
REFERENCES USER2(USER_ID),
CONSTRAINT ORDER2_SHOP_ID_FK FOREIGN KEY (SHOP_ID)
REFERENCES SHOP(SHOP_ID),
CONSTRAINT ORDER2_ORDERED_SEQ_FK FOREIGN KEY (ORDERED_SEQ)
REFERENCES ORDEREDMENU(ORDERED_SEQ)
);
 
create table MENU 
(MENU_SEQ PRIMARY KEY,
SHOP_ID VARCHAR2 (10),
FOOD varchar2(10),
PRICE number(6),
CONSTRAINT MENU_SHOPKEEPER_ID_FK FOREIGN KEY (SHOP_ID)
REFERENCES SHOP(SHOP_ID)
);

create table DELIVERY
(DELIVERY_NUMBER VARCHAR2 (7) PRIMARY KEY,
ORDER_NUMBER NUMBER (7),
CONSTRAINT DELIVERY_ORDER_NUMBER_FK FOREIGN KEY (ORDER_NUMBER)
REFERENCES ORDER2(ORDER_NUMBER));



CREATE TABLE REPLY    
(REPLY_SEQ NUMBER (5) PRIMARY KEY,
REVIEW_SEQ NUMBER (5),
REPLY VARCHAR2 (20),
CONSTRAINT REPLY_REVIEW_NUMBER_FK FOREIGN KEY(REVIEW_SEQ)
REFERENCES REVIEW(REVIEW_SEQ)
);

 create table HYGIENE 
(SHOPNAME VARCHAR2(15) ,
 BUSINESS_LICENSE NUMBER(10) PRIMARY kEY,
 VIOLATION_DETAILS VARCHAR2(10));
 
 CREATE TABLE FOODCATEGORY 
 (UNQ_NUMBER CHAR(2) PRIMARY KEY,
 CATEGORY2 CHAR(8) 
 );
 
CREATE TABLE MENUDETAILS
(ORDERED_SEQ NUMBER (7) PRIMARY KEY,
MENU_SEQ NUMBER (7),
ORDER_NUMBER NUMBER (7),
CONSTRAINT ORDEREDMENU_MENU_SEQ_FK FOREIGN KEY (MENU_SEQ)
REFERENCES MENU (MENU_SEQ),
CONSTRAINT ORDEREDMENU_ORDER_NUMBER_FK FOREIGN KEY (ORDER_NUMBER)
REFERENCES ORDER (ORDER_NUMBER)
); 

 
 
 